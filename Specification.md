...
- **SSML replacement must match plain text structure** (critical for stress marking)
  - Plain text: `$1 метров` → SSML: `$1 метр+ов` (both use full word)
  - Wrong: `$1 метров` + `$1 м+етров` (causes duplication like "метров+етров")
